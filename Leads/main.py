...
def send_email(to_email, to_name):
    ...
    # (Keep your existing HTML body here)
    html_body = f""" 
    <html>
    <body style="font-family: Arial, sans-serif; font-size: 15px; color: #333; line-height: 1.6; margin: 0; padding: 0; background-color: #f6f8fa;">
        <div style="max-width: 600px; margin: auto; padding: 20px; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
...
def run():
    ...
    cursor.execute("""
        ...
        WHERE massemail_unsubscribe COLLATE utf8_general_ci != 'yes'
          AND massemail_email_sent COLLATE utf8_general_ci != 'yes'
        LIMIT 800  # 8 accounts × 100 emails each
    """)
    ...
    for lead in leads:
        ...
        try:
            sender_email = send_email(lead["email"], lead["full_name"])
            cursor.execute("UPDATE leads_new SET massemail_email_sent = 'yes', last_modified = NOW() WHERE id = %s", (lead["id"],))
            conn.commit()
            log_sent(lead["email"], lead["full_name"], sender_email)
            print(f"Sent to {lead['email']} using {sender_email}")
        except Exception as e:
            print(f"Failed to send to {lead['email']}: {e}")
